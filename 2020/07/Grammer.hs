...
happyExpList = Happy_Data_Array.listArray (0,23) ([32,16386,0,384,2,516,1536,0,6144,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Rules","Counts","bag","contain","noBags","nBags","comma","period","%eof"]
        bit_start = st * 12
        bit_end = (st + 1) * 12
...
happyReduction_1 ((HappyAbsSyn4  happy_var_4) `HappyStk`
	...
	_ `HappyStk`
	(HappyTerminal (TokenBag happy_var_1)) `HappyStk`
	happyRest)
	 = HappyAbsSyn4
...
happyReduction_3 ((HappyAbsSyn5  happy_var_4) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (TokenBag happy_var_2)) `HappyStk`
	(HappyTerminal (TokenNBags happy_var_1)) `HappyStk`
	happyRest)
	 = HappyAbsSyn5
...
happyReduction_4 _
	(HappyTerminal (TokenBag happy_var_2))
	(HappyTerminal (TokenNBags happy_var_1))
	 =  HappyAbsSyn5
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenBag happy_dollar_dollar -> cont 6;
	TokenContain -> cont 7;
	TokenNoBags -> cont 8;
	TokenNBags happy_dollar_dollar -> cont 9;
	TokenComma -> cont 10;
	TokenPeriod -> cont 11;
	_ -> happyError' ((tk:tks), [])
	}
